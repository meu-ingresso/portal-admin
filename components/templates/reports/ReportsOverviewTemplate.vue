<template>
  <div class="reports-overview">
    ...
    <v-card v-if="getTickets && getTickets.length > 0" class="mb-6">
      ...
      <v-card-text>
        <v-data-table :headers="ticketsHeaders" :items="ticketsData" :items-per-page="5" class="elevation-1">
          <template #[`item.percentSold`]="{ item }">
            <v-tooltip top>
              <template #activator="{ on, attrs }">
                <v-progress-linear v-bind="attrs" :value="item.percentSold" height="20" color="primary" striped
                  v-on="on">
                  {{ item.percentSold }}%
                </v-progress-linear>
              </template>
    ...
    <v-card v-if="hasSessions" class="mt-6">
      ...
      <v-card-text>
        <v-data-table :headers="sessionsHeaders" :items="sessionsData" :items-per-page="5" class="elevation-1">
          ...
          <template #[`item.percentSold`]="{ item }">
            <v-progress-linear :value="item.percentSold" height="20" color="primary" striped>
              <template #default>
                {{ item.percentSold }}%
              </template>
            </v-progress-linear>
  ...
  computed: {
    ...
    hasSessions() {
      ...
      if (this.getEvent.groups && this.getEvent.groups.length > 0) {
        ...
        const eventsInGroup = this.$store.getters['eventGeneralInfo/$eventList'].filter(event =>
          event.groups &&
          event.groups.length &&
